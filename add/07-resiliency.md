...
Timeouts are bounded on every remote call; Matching to Routing/ETA uses a 100 ms timeout. Bulkheads give each downstream its own connection pool and worker budget, so slow Billing cannot consume Trip Management capacity. Graceful degradation uses Haversine ranking when Routing/ETA is unavailable: match quality drops, but the request path remains available. Redis location data is intentionally rebuildable, while trip and billing state remains durable in PostgreSQL.
