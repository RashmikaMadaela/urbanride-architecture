...
driver location as disposable in-memory data, `Kafka` as a decoupling buffer, dead-letter topics
for poison messages — and decline the machinery built specifically for their scale, such as
...
travel time — most of Grab's allocation quality at a fraction of the build cost. The measured
...
for driver location, eventual consistency for surge pricing (§6.1). Because no ACID transaction
